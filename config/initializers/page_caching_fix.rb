  ...
  module Caching
    ...
    module Pages
      ...
      def cache_page(content = nil, options = nil, gzip = Zlib::BEST_COMPRESSION)
        ...
        logger.warn "BEGIN WRITING PAGE CACHE USING INITIALIZERS/PAGE_CACHING_FIX"
        logger.warn "BEGIN WRITING PAGE CACHE USING INITIALIZERS/PAGE_CACHING_FIX"
        logger.warn "BEGIN WRITING PAGE CACHE USING INITIALIZERS/PAGE_CACHING_FIX"
        logger.warn "BEGIN WRITING PAGE CACHE USING INITIALIZERS/PAGE_CACHING_FIX"
        logger.warn "BEGIN WRITING PAGE CACHE USING INITIALIZERS/PAGE_CACHING_FIX"

        logger.warn "path : " + path.to_s

        #if path == "/"
        #           the_path = '/apache_root/' + request.host.gsub('.', '_') + "/index.html"
        #else
                   the_path = '/apache_root/' + request.host.gsub('.', '_') + path
        #end
       logger.warn "the_path: " + the_path


       logger.warn "END WRITING PAGE CACHE USING INITIALIZERS/PAGE_CACHING_FIX"
       logger.warn "END WRITING PAGE CACHE USING INITIALIZERS/PAGE_CACHING_FIX"
       logger.warn "END WRITING PAGE CACHE USING INITIALIZERS/PAGE_CACHING_FIX"
       logger.warn "END WRITING PAGE CACHE USING INITIALIZERS/PAGE_CACHING_FIX"
       logger.warn "END WRITING PAGE CACHE USING INITIALIZERS/PAGE_CACHING_FIX"
        ...
        self.class.cache_page(content || response.body, the_path, extension, gzip)
      end
...
#           end
#
#    logger.debug "path before: " + path.to_s
#    logger.debug "request.inspect: " + request.inspect
#
#    path = '/' + request.server_name.gsub('.', '_') + path
#
#
#    logger.debug "path after: " + path.to_s
#
#
#    if (type = Mime::LOOKUP[self.content_type]) && (type_symbol = type.symbol).present?
#      extension = ".#{type_symbol}"
...
#def page_cache_path(path, extension = nil)
#
#       page_cache_directory.to_s + '/' + Socket.gethostname.gsub(':', '_') + page_cache_file(path, extension)
#end
#
#
#end