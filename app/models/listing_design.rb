class ListingDesign  < ActiveRecord::Base
...
#FROM         dbo.item
  ...
  def self.search_designs_with_msfte(*args)
        ...
        #search_string =   "EXEC sp_executesql N'SELECT  [listing_designs].* FROM [listing_designs] JOIN CONTAINSTABLE([listing_designs], (*), #{search_terms} ) as msfte_ctbl_0 ON [listing_designs].[id] = msfte_ctbl_0.[key] WHERE  id IN (#{website_search_item_ids}) ORDER BY msfte_ctbl_0.rank DESC'"
        search_string =   "EXEC sp_executesql N'SELECT  [listing_designs].* FROM [listing_designs] JOIN CONTAINSTABLE([listing_designs], (*), #{search_terms} ) as msfte_ctbl_0 ON [listing_designs].[id] = msfte_ctbl_0.[key] ORDER BY msfte_ctbl_0.rank DESC'"
