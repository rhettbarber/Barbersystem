class ListingMerchandise  < ActiveRecord::Base
...
#
#ALTER VIEW [dbo].[listing_merchandises] WITH SCHEMABINDING AS
...
#FROM         dbo.item
  ...
  def self.search_designs_with_msfte(*args)
    ...
    #search_string =   "EXEC sp_executesql N'SELECT  [listing_merchandises].* FROM [listing_merchandises] JOIN CONTAINSTABLE([listing_merchandises], (*), #{search_terms} ) as msfte_ctbl_0 ON [listing_merchandises].[id] = msfte_ctbl_0.[key] WHERE  id IN (#{website_search_item_ids}) ORDER BY msfte_ctbl_0.rank DESC'"

    search_string =   "EXEC sp_executesql N'SELECT  [listing_merchandises].* FROM [listing_merchandises] JOIN CONTAINSTABLE([listing_merchandises], (*), #{search_terms} ) as msfte_ctbl_0 ON [listing_merchandises].[id] = msfte_ctbl_0.[key] ORDER BY msfte_ctbl_0.rank DESC'"




    $stdout.puts "search_string: " + search_string
    #@initial_items = ListingMerchandisesAndMerchandiseOnly.find_with_msfte :matches_any => ['deer', 'hunting' ]

    #ListingMerchandisesAndMerchandiseOnly.transaction do
